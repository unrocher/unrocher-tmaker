...
const APP_VERSION = "V02-touch-toggle-file";
...
function buildPlacementStateFromDesignsData(designId, targetSize) {
  return buildPlacementState({}, designId, targetSize);
}
...
export default function App() {
  ...
  useEffect(() => {
    if (!designId) return;
    const nextPlacement = buildPlacementStateFromDesignsData(designId, fit);
  ...
  function buildPatchEntry(designKey, savedMap) {
    ...
    for (const sizeKey of EDITABLE_SIZES) {
      frontSaved[sizeKey] = roundPlacement(savedMap?.[designKey]?.front?.[sizeKey] ?? defaults?.front);
      backSaved[sizeKey] = roundPlacement(savedMap?.[designKey]?.back?.[sizeKey] ?? defaults?.back);
    }
  ...
  const updatePinchPointers = (e) => {
    pinchRef.current.pointers.set(e.pointerId, { x: e.clientX, y: e.clientY });
  };

  const removePinchPointer = (pointerId) => {
    pinchRef.current.pointers.delete(pointerId);
  };
  ...
  const beginPinchIfNeeded = () => {
    ...
    pinchRef.current.startDistance = Math.max(1, getDistance(p1, p2));
    pinchRef.current.startZoom = zoom;
    pinchRef.current.startPanX = pan.x;
    pinchRef.current.startPanY = pan.y;
    pinchRef.current.startMidX = mid.x;
    pinchRef.current.startMidY = mid.y;
  ...
  const applyPinchTransform = () => {
    const points = Array.from(pinchRef.current.pointers.values());
    if (!pinchRef.current.active || points.length < 2) return;
    const [p1, p2] = points;
    ...
    setZoom(nextZoom);
    setPan({
      x: pinchRef.current.startPanX + (currentMid.x - pinchRef.current.startMidX),
      y: pinchRef.current.startPanY + (currentMid.y - pinchRef.current.startMidY),
    });
  };
  ...
  const onPointerDown = (e) => {
    ...
    if (canEditCurrentSize && hitResizeHandle(logicalPoint, info)) {
      dragRef.current = {
        active: true,
        mode: "resize-se",
        pointerId: e.pointerId,
    ...
    if (canEditCurrentSize && hitDesignBody(logicalPoint, info)) {
      dragRef.current = {
        active: true,
        mode: "move",
        pointerId: e.pointerId,
  ...
  const endDrag = (e) => {
    removePinchPointer(e?.pointerId);

    if (pinchRef.current.pointers.size < 2) {
      pinchRef.current.active = false;
      if (isMobile && pinchRef.current.pointers.size === 1) {
        const remainingPointerId = Array.from(pinchRef.current.pointers.keys())[0];
        const remainingPoint = pinchRef.current.pointers.get(remainingPointerId);
        if (remainingPoint) {
          panRef.current = {
            active: true,
  ...
  const canvasCursor = pinchRef.current.active
    ? "move"
          ...
          <div style={panelStyle(compact)}>
            ...
            >
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: "#78716c", marginTop: 4 }}>{APP_VERSION}</div>
              </div>
              ...
              >
                ...
                <button style={buttonStyle(false, compact)} onClick={exportAllDesignsPatch}>
                  全デザインを書き出し
                </button>
                <button
            ...
            >
              ...
              >
                <div style={{ fontSize: isMobile ? 12 : 13, fontWeight: 800, padding: "0 8px", color: "#44403c" }}>
                  今: {side === "front" ? "表" : "裏"}
                </div>
                <button type="button" style={buttonStyle(false, compact)} onClick={toggleSide}>
                  {side === "front" ? "裏を見る" : "表を見る"}
                </button>
              </div>

              <button style={buttonStyle(false, compact)} onClick={() => zoomByButton(-1)}>
                −
              </button>
              ...
              <button style={buttonStyle(false, compact)} onClick={() => setZoom(1)}>
                100%
              </button>
            </div>
            ...
            {(!isMobile || openSections.designAdjust) && (
              <>
                <div style={{ fontSize: 14, color: "#57534e", lineHeight: 1.7, marginBottom: 14 }}>
                  ・リロード時は designs-data.js を優先
                  <br />
                  ・編集できるのは 120 / M / XXL のみ
                  <br />
                  ・中間サイズは位置のみ補間、版幅は M 基準
                  <br />
                  ・スマホは 2本指で拡大縮小 / 移動、空白1本指で表示移動
                </div>
                ...
                >
                  ...
                  <div style={{ ...inputStyle(compact), background: "#fafaf9", color: "#44403c", fontWeight: 700 }}>
                    種別: {directSaved ? "この表示中の基準点" : "designs-data / 補間"}
                  </div>
                  <div
                    style={{
                      ...inputStyle(compact),
